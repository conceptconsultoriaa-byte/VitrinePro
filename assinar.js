...
const PLANO_NOMES = { basico: "Básico — R$ 49/mês", pro: "Pro — R$ 89/mês" };
...
document.getElementById("cadastroForm").addEventListener("submit", async (e)=>{
  ...
  try{
    ...
    const { data: business, error: bizError } = await supabaseClient.from("businesses")
      .insert({ owner_id: session.user.id, slug, name: nomeNegocio || "Minha Vitrine", segment: "semijoia", subscription_plan: planoSelecionado })
      .select().single();
    ...
    const resp = await fetch(`${BACKEND_URL}/api/assinatura/criar`, {
      ...
      body: JSON.stringify({ business_id: business.id, email, plano: planoSelecionado, produto: "vitrinepro" })
    });
